...
class GreaterThanConstants:
    ...
    def __init__(self, device):
        ...
        # Modern HF tokenizers prepend BOS unless add_special_tokens=False.
        _enc = lambda s: _TOKENIZER.encode(s, add_special_tokens=False)
        for century in range(11, 18):
            all_success = []
            for year in range(century * 100 + 2, (century * 100) + 99):
                a = _enc(f" {year}")
                if a == [_enc(f" {str(year)[:2]}")[0], _enc(str(year)[2:])[0]]:
